  ...
  while (true) {
    if (millis() - systime > 200) {
      ...
      if (usr.distanceCm() < 20 && usv.distanceCm() < 10) {  //vorne u rechts block
        turnForSeconds(3, 40, 1);
      } else if (usr.distanceCm() > 16) {  //rechts lücke
        turnForSeconds(1, 40, 0.5);
        turnForSeconds(4, 40, 1.4);
        turnForSeconds(1, 40, 0.7);
      } else if (usr.distanceCm() > 11 && usr.distanceCm() < 15) {  // zwischen 9 u 11 rechts Korrigieren
        turnForSeconds(4, 30, 0.02);
        turnForSeconds(1, 40, 0.25);
      } else if (usr.distanceCm() > 3 && usr.distanceCm() < 7) {  //zwischen 5 u 7 rechts
