...
void setup() {
  ...
  while (true) {
    ...
    if (ultraschall_rechts.distanceCm() > 15) {
      turnForSeconds(4, 50 / , 0.4);
    }
    if (ultraschall_rechts.distanceCm() < 5) {
      move(5, 30 );
    }
    if (ultraschall_rechts.distanceCm()  >6) {
      move(4, 30 );
    }
    //if (ultraschall_rechts.distanceCm() < 4 && ultraschall_vorne.distanceCm() < 4) {
        //turnForSeconds(3, 50  0.4);
      //}
  //}
  _loop();
