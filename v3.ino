...
void setup() {
  randomSeed((unsigned long)(lightsensor_6.read() * 123456));
  TCCR1A = _BV(WGM10);
  ...
  while (true) {
    if (ultraschall_vorne.distanceCm() > 10) {
      ...
      turnForSeconds(3, 50 / 100.0 * 255, 0.4);  // mBot dreht sich links für 0.4 Sekunden
    }
    if (ultraschall_rechts.distanceCm() > 15) {
      turnForSeconds(4, 50 / 100.0 * 255, 0.4);
    }
    ...
    //if (ultraschall_rechts.distanceCm() < 4 && ultraschall_vorne.distanceCm() < 4) {
        //turnForSeconds(3, 50 / 100.0 * 255, 0.4);
      //}
  //}
  _loop();
}
